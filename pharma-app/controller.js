	...
	return{
		...
		get_statics: function(req, res){
			...
			Fabric_Client.newDefaultKeyValueStore({ path: store_path
				...
				// get the enrolled user from persistence, this user will sign all requests
				return fabricStateStore(state_store);
				var crypto_suite = Fabric_Client.newCryptoSuite();
				// use the same location for the state store (where the users' certificate are kept)
				// and the crypto store (where the users' keys are kept)
				var crypto_store = Fabric_Client.newCryptoKeyStore({path: store_path});
		...
		add_manufacturer: function(req, res){
			...
			Fabric_Client.newDefaultKeyValueStore({ path: store_path
			    ...
			    const request = {
			        ...
			        args: [comp, address, license, owner, cnic, ownerAddress, password],
			        chainId: 'mychannel',
			        txId: tx_id
			    };
		...
		add_distributor: function(req, res){
			...
			Fabric_Client.newDefaultKeyValueStore({ path: store_path
			    ...
			    // recordDistributor - requires 4 args, ID, name, timestamp, address - ex: args: ['10', 'panadol', '1504054225', 'Hansel'],
			    // send proposal to endorser
			    const request = {
			        //targets : --- letting this default to the peers assigned to the channel
			        chaincodeId: 'pharma-app',
		...
		add_chemist: function(req, res){
			...
			Fabric_Client.newDefaultKeyValueStore({ path: store_path
			    ...
			    // recordChemist - requires 4 args, ID, name, timestamp, address - ex: args: ['10', 'panadol', '1504054225', 'Hansel'],
			    // send proposal to endorser
			    const request = {
			        //targets : --- letting this default to the peers assigned to the channel
			        chaincodeId: 'pharma-app',
			        ...
			        if (query_responses[0] instanceof Error) {
						...
						if(temp.id.substring(0,5)!="admin"){
							...
							if(temp.assets==null){
								...
								var newAssets=temp.assets.map( (asset, index)=>({
									index: index+1,
		...
		sell_asset: function(req, res){
			...
			Fabric_Client.newDefaultKeyValueStore({ path: store_path
			    ...
			    var request = {
			        ...
			        chaincodeId: 'pharma-app',
			        fcn: 'sellAsset',
			        args: [assetid, name, phone, timestamp],
			        chainId: 'mychannel',
			        txId: tx_id
			    };
